...
pub fn fromMemory(bytes: []const u8) !Self {
  ...
  const len: c_int = @intCast(bytes.len);
  ...
  return .{
    .width = @intCast(shape.w),
    .height = @intCast(shape.h),
    .channels = @intCast(shape.c),
    .data = data[0..@intCast(shape.w * shape.h * shape.c)],
  };
}
...
pub fn uploadToGPU(self: *const Self, id: *c.GLuint, fmt: c.GLenum, params: []const gl.textures.KeyValue) void {
  ...
  const w: c.GLsizei = @intCast(self.width);
  const h: c.GLsizei = @intCast(self.height);
